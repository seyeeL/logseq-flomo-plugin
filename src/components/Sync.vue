  ...
  setup(props) {
        ...
        switch (syncMode) {
          ...
          case '2': // 日记模式
            ...
            for (let i = 0; i < groupDate.length; i++) {
              ...
              const { memos } = await fetchMemosByDate({
                cookie,
                token,
                server,
                start_date: date,
    ...
    async function loadPageNotes(pageName, memos) {
      ...
      if (pageBlocksTree.length === 0) {
        const firstBlock = await logseq.Editor.insertBlock(pageName, pagePropBlockString, {
          isPageBlock: true,
          format: 'markdown',
        });
      ...
      if (!uuid) {
        pagePropBlock = await logseq.Editor.insertBlock(
          ...
          pagePropBlockString,
          { isPageBlock: false, sibling: true },
        );
    ...
    async function insertBlock(uuid, memos, has_img_memo_id, pageBlocksTree) {
      ...
      for (let j = 0; j < memos.length; j++) {
          ...
          for (let i = 0; i < childrenTree.length; i++) {
            if (childrenTree[i].content.indexOf(`#+flomo_id: ${slug}`) !== -1) {
              ...
              if (childrenTree[i].content.indexOf(`#+updated: ${updated_at}`) !== -1) {
                ...
                if (backlinked_count) {
                  await handleBacklinkeds(
                    slug,
                    img_block_id || childrenTree[i].uuid,
                    img_block_id ? childrenTree[i].uuid : false,
                  );
                }
                break;
              }
        ...
        if (!exportMode && oldUuid) {
          ...
          let n_block = await logseq.Editor.insertBlock(n_uuid, n_content, {
            sibling,
            isPageBlock: false,
            before,
          });
          ...
          // add a blank block
          await logseq.Editor.insertBlock(n_block_id, '', {
            sibling: true,
            isPageBlock: false,
            before: false,
          });
        ...
        if (backlinked_count) {
          await handleBacklinkeds(
            slug,
            img_block_id || n_block_id,
            img_block_id ? n_block_id : false,
          );
        }
      }
    }
    async function handleImages(files, n_block_id) {
      ...
      if (n_BlockTree?.children.length === 0) {
        const res = await logseq.Editor.insertBlock(block_id, imgContent, {
          sibling: false,
          isPageBlock: false,
          before: false,
        });
        ...
        !exportMode &&
        n_BlockTree?.children?.[0].content.indexOf(`#+isImg: true`) !== -1
      ) {
